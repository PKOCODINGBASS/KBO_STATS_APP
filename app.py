...
if _THEME_PATH is None:
    raise ImportError("shared/theme.py introuvable à côté de l'app KBO.")
...
def _match_a_commence_kbo(statut) -> bool:
    """True dès STARTED / LIVE / RESULT (plus en phase BEFORE)."""
    s = (statut or '').strip().upper()
    return s in ('STARTED', 'LIVE', 'RESULT', 'SUSPENDED')
...
def _cle_snapshot_match(m: dict) -> str:
    ...
    if gid is not None and gid != '':
        return f"gid:{gid}"
    return f"teams:{m.get('code_home')}|{m.get('code_away')}|{m.get('home_name')}|{m.get('away_name')}|{m.get('nom_home')}|{m.get('nom_away')}"
...
def _fusionner_snapshots_figes(existants: list, nouveaux: list, maintenant_iso: str) -> list:
    ...
    for m in nouveaux or []:
        ...
        a_commence = _match_a_commence_kbo(m.get('statut'))
...
def _sauvegarder_predictions_du_jour(date_str: str, matches_snapshot: list) -> None:
    ...
    try:
        ...
        maintenant_iso = datetime.now(TZ_SEOUL).isoformat()
...
def _classer_candidats_home_runs_kbo(candidats: list) -> pd.DataFrame:
...
def _calculer_top5_home_runs_kbo(candidats: list) -> pd.DataFrame:
    ...
    """
    df = _classer_candidats_home_runs_kbo(candidats)
    if df.empty:
        return df
    return df.head(5).reset_index(drop=True)


def _classer_candidats_runs_kbo(candidats: list) -> pd.DataFrame:
...
def _calculer_top5_runs_kbo(candidats: list) -> pd.DataFrame:
    ...
    """
    df = _classer_candidats_runs_kbo(candidats)
    if df.empty:
        return df
    return df.head(5).reset_index(drop=True)
...
def _top_candidats_hr_kbo(lineup_equipe: dict, dict_noms_anglais: dict, n: int = 3) -> list:
    """
...
def _top_candidats_runs_kbo(lineup_equipe: dict, dict_noms_anglais: dict, n: int = 3) -> list:
    """
...
def construire_donnees_hot_pronostics_kbo(annee: int):
    ...
    df_hr_all = _classer_candidats_home_runs_kbo(candidats_hr)
    df_runs_all = _classer_candidats_runs_kbo(candidats_runs)
    ...
    for m, ligne_victoire in zip(matchs_du_jour, lignes_victoire):
        home, away = m['nom_home'], m['nom_away']
...
def _nb_marque_joueur_kbo(pred_item, scoreurs: list) -> int:
    ...
    for item in scoreurs or []:
        ...
        try:
            ...
            n = 0
        if n > 0 and nom_predit and _noms_joueurs_correspondent_kbo(nom_predit, nom_reel):
            return n
    return 0


def _preds_bilan_depuis_snapshot_kbo(pred, kind: str) -> list:
...
def _bilan_joueurs_predits_kbo(preds, scoreurs_home, scoreurs_away, label: str):
...
def formater_recommandation_totaux_over_under(total_projete, ligne):
    ...
    Pour la cohérence inter-onglets, passer la projection MATCH (`_total_runs_predit_kbo`).
    """
    if total_projete is None or ligne is None:
        return None
    try:
...
def assembler_lignes_recap_hot_pronostics(
    ...
    date_str = datetime.now(TZ_SEOUL).strftime('%Y-%m-%d')
    ...
    for idx, m in enumerate(matchs_jour):
        ...
        fige = bool(snap and (snap.get('fige') or _match_a_commence_kbo(m.get('statut'))))
        if fige and snap:
        ...
        runs_home = moyennes.get(m.get('code_home'))
        runs_away = moyennes.get(m.get('code_away'))
        total_proj = (
            snap.get('total_runs_predit') if fige and snap and snap.get('total_runs_predit') is not None
            else _total_runs_predit_kbo(runs_home, runs_away)
        )
...
def construire_bilan_veille_kbo(annee: int):
    ...
    for _, g in df_hier.iterrows():
        ...
        texte_hr, icone_hr = _bilan_joueurs_predits_kbo(
            _preds_bilan_depuis_snapshot_kbo(pred, 'hr'),
            hr_home, hr_away, "HR",
        )
        preds_run = _preds_bilan_depuis_snapshot_kbo(pred, 'run')
        if not preds_run and pred:
            h, a = _candidats_runs_bilan_kbo(pred, annee)
...
def construire_resume_matchs_du_jour_kbo(annee: int, cache_bust: int = 0):
    ...
    # les deux appels).
    try:
        matchs_lineups, _, _, df_victoires, *_ = construire_donnees_hot_pronostics_kbo(annee)
    except Exception:
        ...
        if annee != ANNEE_COURANTE:
            ...
            with st.spinner(
                ...
                matchs_jour, df_top5_hr, df_top5_runs, df_victoires, df_hr_all, df_runs_all = (
                    construire_donnees_hot_pronostics_kbo(annee)
                )
            ...
            if not matchs_jour:
                ...
                afficher_assistant_hot_pronostics(
                    df_hr_all, df_runs_all, df_victoires, lignes_recap, key_prefix="kbo_hot"
                )
                st.caption(
...
with onglets[2]:
    ...
    with col1:
        options_equipes = [
            f"{abbr} - {nom}"
            for abbr, nom in sorted(EQUIPES_KBO.items(), key=lambda x: x[1].casefold())
        ]
...
with onglets[3]:
    ...
    if annee != ANNEE_COURANTE:
        ...
        if not match_du_jour:
            ...
            ligne_ou = obtenir_ligne_over_under_saison_kbo(annee)
            moyennes_match = obtenir_moyennes_runs_10_toutes_equipes(annee)
            total_match_hot = _total_runs_predit_kbo(
                moyennes_match.get(match_du_jour.get('code_home')),
                moyennes_match.get(match_du_jour.get('code_away')),
            )
            total_vue_equipe = (
